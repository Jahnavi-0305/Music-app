...
import { MusicContext } from '../utilities/store.js';
...
const PlaylistsScreen = ({ route, navigation }) => {
  ...
  const createPlaylist = async () => {
    if (inputValue === "")
      ...
      if (playlistExists) {
        window.alert("Playlist with the name " + inputValue + " already exists!");
      }
      else {
      ...
      <Modal visible={modalVisible} onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modal}>
          ...
          <View style={{flexDirection: "row", marginTop: 10}}>
            <TouchableOpacity
              activeOpacity={0.7}
              onPress={() => {setInputValue(''); setModalVisible(false);}}
            >
              <View style={[styles.createPlaylistButton, {width: wp(25), height: hp(6.5)}]}>
            ...
            >
              <View style={[styles.createPlaylistButton, {width: wp(42), height: hp(6.5), marginLeft: wp(4)}]}>
    ...
    return (
      ...
      >
        <View style={styles.playlistButton}>
          ...
          <Entypo name='cross' onPress={() => deletePlaylist(item.name)} size={wp(9)}/>
        </View>
      </TouchableOpacity>
    );
  }
